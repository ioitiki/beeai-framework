...
class Emitter extends serializable_cjs.Serializable {
  ...
  match(matcher, callback, options) {
    const createMatcher = /* @__PURE__ */ __name(() => {
      const matchers = [];
      let matchNested = options?.matchNested;
      if (matcher === "*") {
        matchNested ??= false;
        matchers.push((event) => event.path === utils_cjs.createFullPath(this.namespace, event.name));
      } else if (matcher === "*.*") {
        matchNested ??= true;
        matchers.push(() => true);
        ...
        if (utils_cjs.isPath(matcher)) {
          matchNested ??= true;
          matchers.push((event) => event.path === matcher);
        } else {
          matchNested ??= false;
          matchers.push((event) => event.name === matcher && event.path === utils_cjs.createFullPath(this.namespace, event.name));
        }
      } else {
        throw new errors_cjs.EmitterError("Invalid matcher provided!");
      }
      if (!matchNested) {
        const matchSameRun = /* @__PURE__ */ __name((event) => {
          return !this.trace?.runId ? true : this.trace?.runId === event.trace?.runId;
        }, "matchSameRun");
        matchers.unshift(matchSameRun);
      }
      return (event) => matchers.every((matcher2) => matcher2(event));
    }, "createMatcher");
    const listener = {
      options,
      callback,
