...
class PromptTemplate extends serializable_cjs.Serializable {
  ...
  fork(customizer) {
    const config = remeda.clone(this.config);
    const newConfig = customizer?.(config) ?? config;
    if (!remeda.isPlainObject(newConfig)) {
      throw new errors_cjs.ValueError("Return type from customizer must be a config or nothing.");
    }
  ...
  render(input) {
    ...
    Object.assign(updatedInput, remeda.pickBy(this.config.defaults, (_, k) => object_cjs.getProp(updatedInput, [
      k
    ]) === void 0));
