...
class StreamlitAgent extends base_cjs.BaseAgent {
  ...
  async _run(input, _options, run) {
    ...
    const assistantMessage = new message_cjs.AssistantMessage(content);
  ...
  async prepare(input) {
    const systemMessage = new message_cjs.SystemMessage((this.input.templates?.system ?? prompts_cjs.StreamlitAgentSystemPrompt).render({}));
    const userMessage = input.prompt !== null || this.memory.isEmpty() ? new message_cjs.UserMessage(input.prompt ?? "No message.", {
