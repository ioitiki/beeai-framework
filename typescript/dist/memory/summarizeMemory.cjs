...
class SummarizeMemory extends base_cjs.BaseMemory {
  ...
  get messages() {
    ...
    return [
      new message_cjs.AssistantMessage(currentSummary)
    ];
  }
  ...
  async add(message, _index) {
    const response = await this.llm.create({
      messages: [
        new message_cjs.SystemMessage(this.template.render({
          summary: this.summary
        })),
        new message_cjs.AssistantMessage(`New lines of conversation:
