...
class FileCache extends base_cjs.BaseCache {
  ...
  static async fromProvider(provider, input) {
    await fs__default.default.promises.writeFile(input.fullPath, await provider.serialize());
  ...
  async save() {
    ...
    return await fs__default.default.promises.writeFile(this.input.fullPath, await provider.serialize());
  }
  async size() {
    const provider = await this.getProvider();
