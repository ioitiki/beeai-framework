  ...
  async _run({ query: input }, _options) {
    ...
    const results = await Promise.all(bestCandidates.map(async ({ pageId }) => {
      ...
      return promise_cjs.asyncProperties({
        ...
        url: page.fullurl,
        fields: promise_cjs.asyncProperties(R.mapValues(runOptions?.extraction?.fields ?? {}, (value, key) => this._mappers[key](page, runOptions).then((response) => value.transform ? value.transform(response) : response).catch(() => void 0)))
      });
    }));
